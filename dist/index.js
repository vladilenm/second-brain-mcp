...
// ── Tool: healthcheck ───────────────────────────────────────
server.tool('healthcheck', 'Check vault accessibility and return statistics: note counts by type/status, active projects, areas, decisions', {}, async () => {
    try {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ status: 'ok', vaultPath: VAULT_PATH, ...stats }, null, 2),
                },
            ],
        };
    }
...
server.tool('search_knowledge', 'Semantic search across vault: full-text + frontmatter matching. Filter by type, status, tags. Returns ranked results with snippets.', {
...
// ── Tool: get_note ──────────────────────────────────────────
server.tool('get_note', 'Read a single note by path or name. Returns full content with parsed YAML frontmatter.', {
    identifier: z
        .string()
    ...
    return {
        content: [
            {
                type: 'text',
                text: JSON.stringify({ path: note.path, name: note.name, frontmatter: note.frontmatter, content: note.content }, null, 2),
            },
        ],
    };
});
// ── Tool: list_projects ─────────────────────────────────────
server.tool('list_projects', 'List all projects with status, deadline, and tags. Optionally filter by status (active/paused/done/someday).', {
    status: z
        .enum(['active', 'paused', 'done', 'someday'])
        .optional()
...
server.tool('get_project_context', 'Full context for a project: README content + related notes (via wikilinks) + open tasks + linked decisions. The key tool for understanding what a project is about.', {
    project: z
        .string()
...
// ── Tool: find_related ──────────────────────────────────────
server.tool('find_related', 'Find notes related to a given note via: frontmatter `related:`, wikilinks, backlinks, shared tags. Ranked by connection strength.', {
...
// ── Tool: extract_tasks ─────────────────────────────────────
server.tool('extract_tasks', 'Extract checkbox tasks (- [ ] / - [x]) from vault notes. Optionally scope to a folder. By default returns only open tasks.', {
    folder: z
        .string()
        .optional()
...
server.tool('extract_decisions', 'Extract decisions from decision log folders (02_Areas/*/Decisions/). Optionally filter by area name. Sorted newest first.', {
    area: z
        .string()
        .optional()
